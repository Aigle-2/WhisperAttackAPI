...
namespace VaivoxServerCommand
{
    ...
    public class VA_Plugin
    {
        ...
        /// <c>vaProxy.Command</c> surface. Kept tiny so all decision logic stays in
        /// the testable <see cref="Decide"/>.
        /// </summary>
        private sealed class VaProxyCommandProbe : ICommandProbe
        {
            ...
            {
                _vaProxy.Command.Execute(text, true, true);
            }
        }
        ...
        private static async Task HandleVaivoxCommand(dynamic vaProxy, TcpClient client)
        {
            ...
            using (NetworkStream stream = client.GetStream())
            {
                try
                {
                    ...
                    MatchResult result = Decide(new VaProxyCommandProbe(vaProxy), receivedMessage);
                    ...
                    // match outcome. Best-effort — an old app that does not read the
                    // reply makes this Write hit a broken pipe; tolerate it (compat
                    // matrix) so the plugin keeps working against any app version.
                    try
                    {
